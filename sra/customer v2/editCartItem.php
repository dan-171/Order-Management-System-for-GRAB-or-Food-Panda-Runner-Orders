...
if ($id) {
  $foodStmt = $pdo->prepare("
		SELECT oi.ID, oi.Order_ID, f.Name AS name, f.Type AS type, f.Price AS price, oi.Quantity AS quantity, oi.Subtotal AS subtotal
		FROM order_items oi
	...
	if (!$cartItem) {
		$drinkStmt = $pdo->prepare("
			SELECT oi.ID, oi.Order_ID, d.Name AS name, d.hotPrice, d.coldPrice, oi.Type AS type, oi.Quantity AS quantity, oi.Subtotal AS subtotal
			FROM order_items oi
			JOIN orders o ON oi.Order_ID = o.ID
	...
	if (!$cartItem){
		$addonStmt = $pdo->prepare("
			SELECT oi.ID, oi.Order_ID, a.Name AS name, a.Price AS price, oi.Quantity AS quantity, oi.Subtotal AS subtotal
			FROM order_items oi
			JOIN orders o ON oi.Order_ID = o.ID
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$oldSubtotal = $cartItem['subtotal'];
	
	if ($cartItem) {
		if (isset($cartItem['price']))
			$newSubtotal = $cartItem['price'] * $newQty;
		else if (isset($cartItem['hotPrice']) && isset($cartItem['coldPrice'])){
	...
	if ($newQty > 0) {
		...
		$update->execute([$newQty, $newSubtotal, $itemID]);
		
		if ($update->rowCount() > 0) {
			$updateOrder = $pdo->prepare("UPDATE orders SET Total_Amount = Total_Amount + ? WHERE ID = ?");
			$updateOrder->execute([$difference, $orderID]);
		}
	} else {
		...
		if ($del->rowCount() > 0) {
			$updateOrder = $pdo->prepare("UPDATE orders SET Total_Amount = Total_Amount - ? WHERE ID = ?");
			$updateOrder->execute([$oldSubtotal, $orderID]);
		}
	}
	...
	<div style="width:300px; margin:50px auto; background:white; padding:20px; text-align:center; border-radius:8px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
		...
		<?php if ($cartItem): ?>
			...
			<form method="POST" action="" onsubmit="return confirm('Remove this item?');">
				...
				<input type="submit" value="Remove Item" style="background:none; border:none; color:red; text-decoration:underline; cursor:pointer;">
			</form>
