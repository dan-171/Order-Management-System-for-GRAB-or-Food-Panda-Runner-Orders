...
$stmt = $conn->prepare("SELECT full_name, username, email, address, phone_number FROM customer WHERE username = ?");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['update_details'])) {
        ...
        if ($newUsername !== $currentUser) {
            $checkStmt = $conn->prepare("SELECT id FROM customer WHERE username = ?");
        ...
        if (!$error) {
            $updateStmt = $conn->prepare("UPDATE customer SET address = ?, username = ?, phone_number = ? WHERE username = ?");
            $updateStmt->bind_param("ssss", $newAddress, $newUsername, $newPhone, $currentUser);
            
            if ($updateStmt->execute()) {
                $message = "Profile details updated!";
                $_SESSION['username_unique'] = $newUsername;
                $currentUser = $newUsername; 
                
                $userData['username'] = $newUsername;
                $userData['address'] = $newAddress;
                $userData['phone_number'] = $newPhone;
            } else {
    ...
    // --- CASE B: DELETE ACCOUNT ---
    if (isset($_POST['delete_account'])) {
        $delStmt = $conn->prepare("DELETE FROM customer WHERE username = ?");
        $delStmt->bind_param("s", $currentUser);
        if ($delStmt->execute()) {
            session_destroy();
            echo "<script>
...
<div class="profile-container">
    ...
    <form method="POST" action="">
        <div class="info-group">
            ...
            <input type="text" value="<?php echo htmlspecialchars($userData['full_name']); ?>" class="readonly-field" readonly>
        </div>
        ...
        <div class="info-group">
            <label>Email</label>
            <input type="email" value="<?php echo htmlspecialchars($userData['email']); ?>" class="readonly-field" readonly>
        </div>
        ...
        <div class="info-group">
            ...
            <textarea name="address" rows="3" placeholder="Enter your delivery address"><?php echo htmlspecialchars($userData['address']); ?></textarea>
        </div>
